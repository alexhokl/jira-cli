/*
Jira Software Cloud API

Jira Software Cloud REST API documentation

API version: 1001.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package swagger_software

import (
	"encoding/json"
)

// checks if the StatusCategory type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &StatusCategory{}

// StatusCategory A status category.
type StatusCategory struct {
	// The name of the color used to represent the status category.
	ColorName *string `json:"colorName,omitempty"`
	// The ID of the status category.
	Id *int64 `json:"id,omitempty"`
	// The key of the status category.
	Key *string `json:"key,omitempty"`
	// The name of the status category.
	Name *string `json:"name,omitempty"`
	// The URL of the status category.
	Self *string `json:"self,omitempty"`
	AdditionalProperties map[string]interface{}
}

type _StatusCategory StatusCategory

// NewStatusCategory instantiates a new StatusCategory object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewStatusCategory() *StatusCategory {
	this := StatusCategory{}
	return &this
}

// NewStatusCategoryWithDefaults instantiates a new StatusCategory object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewStatusCategoryWithDefaults() *StatusCategory {
	this := StatusCategory{}
	return &this
}

// GetColorName returns the ColorName field value if set, zero value otherwise.
func (o *StatusCategory) GetColorName() string {
	if o == nil || IsNil(o.ColorName) {
		var ret string
		return ret
	}
	return *o.ColorName
}

// GetColorNameOk returns a tuple with the ColorName field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StatusCategory) GetColorNameOk() (*string, bool) {
	if o == nil || IsNil(o.ColorName) {
		return nil, false
	}
	return o.ColorName, true
}

// HasColorName returns a boolean if a field has been set.
func (o *StatusCategory) HasColorName() bool {
	if o != nil && !IsNil(o.ColorName) {
		return true
	}

	return false
}

// SetColorName gets a reference to the given string and assigns it to the ColorName field.
func (o *StatusCategory) SetColorName(v string) {
	o.ColorName = &v
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *StatusCategory) GetId() int64 {
	if o == nil || IsNil(o.Id) {
		var ret int64
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StatusCategory) GetIdOk() (*int64, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *StatusCategory) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given int64 and assigns it to the Id field.
func (o *StatusCategory) SetId(v int64) {
	o.Id = &v
}

// GetKey returns the Key field value if set, zero value otherwise.
func (o *StatusCategory) GetKey() string {
	if o == nil || IsNil(o.Key) {
		var ret string
		return ret
	}
	return *o.Key
}

// GetKeyOk returns a tuple with the Key field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StatusCategory) GetKeyOk() (*string, bool) {
	if o == nil || IsNil(o.Key) {
		return nil, false
	}
	return o.Key, true
}

// HasKey returns a boolean if a field has been set.
func (o *StatusCategory) HasKey() bool {
	if o != nil && !IsNil(o.Key) {
		return true
	}

	return false
}

// SetKey gets a reference to the given string and assigns it to the Key field.
func (o *StatusCategory) SetKey(v string) {
	o.Key = &v
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *StatusCategory) GetName() string {
	if o == nil || IsNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StatusCategory) GetNameOk() (*string, bool) {
	if o == nil || IsNil(o.Name) {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *StatusCategory) HasName() bool {
	if o != nil && !IsNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *StatusCategory) SetName(v string) {
	o.Name = &v
}

// GetSelf returns the Self field value if set, zero value otherwise.
func (o *StatusCategory) GetSelf() string {
	if o == nil || IsNil(o.Self) {
		var ret string
		return ret
	}
	return *o.Self
}

// GetSelfOk returns a tuple with the Self field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StatusCategory) GetSelfOk() (*string, bool) {
	if o == nil || IsNil(o.Self) {
		return nil, false
	}
	return o.Self, true
}

// HasSelf returns a boolean if a field has been set.
func (o *StatusCategory) HasSelf() bool {
	if o != nil && !IsNil(o.Self) {
		return true
	}

	return false
}

// SetSelf gets a reference to the given string and assigns it to the Self field.
func (o *StatusCategory) SetSelf(v string) {
	o.Self = &v
}

func (o StatusCategory) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o StatusCategory) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.ColorName) {
		toSerialize["colorName"] = o.ColorName
	}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !IsNil(o.Key) {
		toSerialize["key"] = o.Key
	}
	if !IsNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	if !IsNil(o.Self) {
		toSerialize["self"] = o.Self
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *StatusCategory) UnmarshalJSON(data []byte) (err error) {
	varStatusCategory := _StatusCategory{}

	err = json.Unmarshal(data, &varStatusCategory)

	if err != nil {
		return err
	}

	*o = StatusCategory(varStatusCategory)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "colorName")
		delete(additionalProperties, "id")
		delete(additionalProperties, "key")
		delete(additionalProperties, "name")
		delete(additionalProperties, "self")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableStatusCategory struct {
	value *StatusCategory
	isSet bool
}

func (v NullableStatusCategory) Get() *StatusCategory {
	return v.value
}

func (v *NullableStatusCategory) Set(val *StatusCategory) {
	v.value = val
	v.isSet = true
}

func (v NullableStatusCategory) IsSet() bool {
	return v.isSet
}

func (v *NullableStatusCategory) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableStatusCategory(val *StatusCategory) *NullableStatusCategory {
	return &NullableStatusCategory{value: val, isSet: true}
}

func (v NullableStatusCategory) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableStatusCategory) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


