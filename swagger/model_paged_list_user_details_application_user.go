/*
The Jira Cloud platform REST API

Jira Cloud platform REST API documentation

API version: 1001.0.0-SNAPSHOT-e4f05783a723054bc221d0780e48a2dc2ed25997
Contact: ecosystem@atlassian.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package swagger

import (
	"encoding/json"
)

// checks if the PagedListUserDetailsApplicationUser type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PagedListUserDetailsApplicationUser{}

// PagedListUserDetailsApplicationUser A paged list. To access additional details append `[start-index:end-index]` to the expand request. For example, `?expand=sharedUsers[10:40]` returns a list starting at item 10 and finishing at item 40.
type PagedListUserDetailsApplicationUser struct {
	// The index of the last item returned on the page.
	EndIndex *int32 `json:"end-index,omitempty"`
	// The list of items.
	Items []UserDetails `json:"items,omitempty"`
	// The maximum number of results that could be on the page.
	MaxResults *int32 `json:"max-results,omitempty"`
	// The number of items on the page.
	Size *int32 `json:"size,omitempty"`
	// The index of the first item returned on the page.
	StartIndex *int32 `json:"start-index,omitempty"`
}

// NewPagedListUserDetailsApplicationUser instantiates a new PagedListUserDetailsApplicationUser object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPagedListUserDetailsApplicationUser() *PagedListUserDetailsApplicationUser {
	this := PagedListUserDetailsApplicationUser{}
	return &this
}

// NewPagedListUserDetailsApplicationUserWithDefaults instantiates a new PagedListUserDetailsApplicationUser object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPagedListUserDetailsApplicationUserWithDefaults() *PagedListUserDetailsApplicationUser {
	this := PagedListUserDetailsApplicationUser{}
	return &this
}

// GetEndIndex returns the EndIndex field value if set, zero value otherwise.
func (o *PagedListUserDetailsApplicationUser) GetEndIndex() int32 {
	if o == nil || IsNil(o.EndIndex) {
		var ret int32
		return ret
	}
	return *o.EndIndex
}

// GetEndIndexOk returns a tuple with the EndIndex field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PagedListUserDetailsApplicationUser) GetEndIndexOk() (*int32, bool) {
	if o == nil || IsNil(o.EndIndex) {
		return nil, false
	}
	return o.EndIndex, true
}

// HasEndIndex returns a boolean if a field has been set.
func (o *PagedListUserDetailsApplicationUser) HasEndIndex() bool {
	if o != nil && !IsNil(o.EndIndex) {
		return true
	}

	return false
}

// SetEndIndex gets a reference to the given int32 and assigns it to the EndIndex field.
func (o *PagedListUserDetailsApplicationUser) SetEndIndex(v int32) {
	o.EndIndex = &v
}

// GetItems returns the Items field value if set, zero value otherwise.
func (o *PagedListUserDetailsApplicationUser) GetItems() []UserDetails {
	if o == nil || IsNil(o.Items) {
		var ret []UserDetails
		return ret
	}
	return o.Items
}

// GetItemsOk returns a tuple with the Items field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PagedListUserDetailsApplicationUser) GetItemsOk() ([]UserDetails, bool) {
	if o == nil || IsNil(o.Items) {
		return nil, false
	}
	return o.Items, true
}

// HasItems returns a boolean if a field has been set.
func (o *PagedListUserDetailsApplicationUser) HasItems() bool {
	if o != nil && !IsNil(o.Items) {
		return true
	}

	return false
}

// SetItems gets a reference to the given []UserDetails and assigns it to the Items field.
func (o *PagedListUserDetailsApplicationUser) SetItems(v []UserDetails) {
	o.Items = v
}

// GetMaxResults returns the MaxResults field value if set, zero value otherwise.
func (o *PagedListUserDetailsApplicationUser) GetMaxResults() int32 {
	if o == nil || IsNil(o.MaxResults) {
		var ret int32
		return ret
	}
	return *o.MaxResults
}

// GetMaxResultsOk returns a tuple with the MaxResults field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PagedListUserDetailsApplicationUser) GetMaxResultsOk() (*int32, bool) {
	if o == nil || IsNil(o.MaxResults) {
		return nil, false
	}
	return o.MaxResults, true
}

// HasMaxResults returns a boolean if a field has been set.
func (o *PagedListUserDetailsApplicationUser) HasMaxResults() bool {
	if o != nil && !IsNil(o.MaxResults) {
		return true
	}

	return false
}

// SetMaxResults gets a reference to the given int32 and assigns it to the MaxResults field.
func (o *PagedListUserDetailsApplicationUser) SetMaxResults(v int32) {
	o.MaxResults = &v
}

// GetSize returns the Size field value if set, zero value otherwise.
func (o *PagedListUserDetailsApplicationUser) GetSize() int32 {
	if o == nil || IsNil(o.Size) {
		var ret int32
		return ret
	}
	return *o.Size
}

// GetSizeOk returns a tuple with the Size field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PagedListUserDetailsApplicationUser) GetSizeOk() (*int32, bool) {
	if o == nil || IsNil(o.Size) {
		return nil, false
	}
	return o.Size, true
}

// HasSize returns a boolean if a field has been set.
func (o *PagedListUserDetailsApplicationUser) HasSize() bool {
	if o != nil && !IsNil(o.Size) {
		return true
	}

	return false
}

// SetSize gets a reference to the given int32 and assigns it to the Size field.
func (o *PagedListUserDetailsApplicationUser) SetSize(v int32) {
	o.Size = &v
}

// GetStartIndex returns the StartIndex field value if set, zero value otherwise.
func (o *PagedListUserDetailsApplicationUser) GetStartIndex() int32 {
	if o == nil || IsNil(o.StartIndex) {
		var ret int32
		return ret
	}
	return *o.StartIndex
}

// GetStartIndexOk returns a tuple with the StartIndex field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PagedListUserDetailsApplicationUser) GetStartIndexOk() (*int32, bool) {
	if o == nil || IsNil(o.StartIndex) {
		return nil, false
	}
	return o.StartIndex, true
}

// HasStartIndex returns a boolean if a field has been set.
func (o *PagedListUserDetailsApplicationUser) HasStartIndex() bool {
	if o != nil && !IsNil(o.StartIndex) {
		return true
	}

	return false
}

// SetStartIndex gets a reference to the given int32 and assigns it to the StartIndex field.
func (o *PagedListUserDetailsApplicationUser) SetStartIndex(v int32) {
	o.StartIndex = &v
}

func (o PagedListUserDetailsApplicationUser) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PagedListUserDetailsApplicationUser) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.EndIndex) {
		toSerialize["end-index"] = o.EndIndex
	}
	if !IsNil(o.Items) {
		toSerialize["items"] = o.Items
	}
	if !IsNil(o.MaxResults) {
		toSerialize["max-results"] = o.MaxResults
	}
	if !IsNil(o.Size) {
		toSerialize["size"] = o.Size
	}
	if !IsNil(o.StartIndex) {
		toSerialize["start-index"] = o.StartIndex
	}
	return toSerialize, nil
}

type NullablePagedListUserDetailsApplicationUser struct {
	value *PagedListUserDetailsApplicationUser
	isSet bool
}

func (v NullablePagedListUserDetailsApplicationUser) Get() *PagedListUserDetailsApplicationUser {
	return v.value
}

func (v *NullablePagedListUserDetailsApplicationUser) Set(val *PagedListUserDetailsApplicationUser) {
	v.value = val
	v.isSet = true
}

func (v NullablePagedListUserDetailsApplicationUser) IsSet() bool {
	return v.isSet
}

func (v *NullablePagedListUserDetailsApplicationUser) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePagedListUserDetailsApplicationUser(val *PagedListUserDetailsApplicationUser) *NullablePagedListUserDetailsApplicationUser {
	return &NullablePagedListUserDetailsApplicationUser{value: val, isSet: true}
}

func (v NullablePagedListUserDetailsApplicationUser) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePagedListUserDetailsApplicationUser) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


